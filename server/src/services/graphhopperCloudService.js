 ...
 */
  async getRoute(params) {
    ...
    const {
      points,
      profile = 'foot',
      algorithm = 'auto', // ✅ Changé de 'dijkstra' à 'auto'
    ...
    try {
      ...
      if (algorithm === 'round_trip') {
        ...
        // ✅ Pour les routes simples, ne pas spécifier d'algorithme
        // ou désactiver CH si algorithme spécifique requis
        if (algorithm && algorithm !== 'auto') {
          requestParams.algorithm = algorithm;
          // ✅ Désactiver Contraction Hierarchies pour algorithmes spécifiques
          requestParams.ch = false;
          requestParams.lm = false; // Désactiver aussi Landmarks
        }
        // Si algorithm === 'auto', on laisse GraphHopper choisir (par défaut CH)
      }

      // Éviter le trafic si demandé (désactive automatiquement CH)
      if (avoidTraffic) {
        requestParams.ch = false;
